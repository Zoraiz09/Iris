      ...
      <main className="px-6 py-6 w-full">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Moisture Card */}
            <div className={`${sensorData?.moisture < 60 || sensorData?.moisture > 70 ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'} rounded-lg shadow-md p-4 text-center border-2`}>
              <div className="flex justify-center mb-2">
                <svg className={`w-8 h-8 ${sensorData?.moisture < 60 || sensorData?.moisture > 70 ? 'text-red-500' : 'text-green-500'}`} fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 2a1 1 0 00-.894.553L7.382 6H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V8a1 1 0 100-2h-3.382l-1.724-3.447A1 1 0 0010 2zM6 8v10h8V8H6z" clipRule="evenodd" />
                  <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                </svg>
              </div>
              <p className="text-gray-700 font-semibold">Moisture</p>
              <p className={`${sensorData?.moisture < 60 || sensorData?.moisture > 70 ? 'text-red-500' : 'text-green-500'} text-2xl font-bold`}>
                {sensorData?.moisture?.toFixed(1) || '0.0'}%
              </p>
              <p className="text-gray-500 text-sm">Optimal: 60-70%</p>
            </div>

            {/* pH Level Card */}
            <div className={`${sensorData?.ph_level < 6.0 || sensorData?.ph_level > 7.5 ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'} rounded-lg shadow-md p-4 text-center border-2`}>
              <div className="flex justify-center mb-2">
                <svg className={`w-8 h-8 ${sensorData?.ph_level < 6.0 || sensorData?.ph_level > 7.5 ? 'text-red-500' : 'text-green-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 11.5V14m0-2.5v-6a1.5 1.5 0 113 0v6m-3 6h6m-9 0h9M7 14h6m1.5-1.5a1.5 1.5 0 113 0v6m-3-6H10m6 0h-6" />
                </svg>
              </div>
              <p className="text-gray-700 font-semibold">pH Level</p>
              <p className={`${sensorData?.ph_level < 6.0 || sensorData?.ph_level > 7.5 ? 'text-red-500' : 'text-green-500'} text-2xl font-bold`}>
                {sensorData?.ph_level?.toFixed(1) || '0.0'}
              </p>
              <p className="text-gray-500 text-sm">Optimal: 6.0-7.5</p>
            </div>

            {/* NPK Levels Card */}
            <div className={`${(sensorData?.nitrogen === 0 && sensorData?.phosphorus === 0 && sensorData?.potassium === 0) || 
              (sensorData?.nitrogen < 25 || sensorData?.phosphorus < 15 || sensorData?.potassium < 20) ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'} rounded-lg shadow-md p-4 text-center border-2`}>
              <div className="flex justify-center mb-2">
                <svg className={`w-8 h-8 ${(sensorData?.nitrogen === 0 && sensorData?.phosphorus === 0 && sensorData?.potassium === 0) || 
                  (sensorData?.nitrogen < 25 || sensorData?.phosphorus < 15 || sensorData?.potassium < 20) ? 'text-red-500' : 'text-green-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <p className="text-gray-700 font-semibold">NPK Levels</p>
              <div className="flex justify-center space-x-6 text-xl font-bold mt-2 mb-1">
              ...
              <div className="flex justify-center space-x-6 text-2xl font-bold">
                <p className={`${(sensorData?.nitrogen === 0) || (sensorData?.nitrogen < 25) ? 'text-red-500' : 'text-green-500'}`}>{sensorData?.nitrogen || 0}</p>
                <p className={`${(sensorData?.phosphorus === 0) || (sensorData?.phosphorus < 15) ? 'text-red-500' : 'text-green-500'}`}>{sensorData?.phosphorus || 0}</p>
                <p className={`${(sensorData?.potassium === 0) || (sensorData?.potassium < 20) ? 'text-red-500' : 'text-green-500'}`}>{sensorData?.potassium || 0}</p>
              </div>
            </div>

            {/* Root Rot Prediction Card */}
            <div className={`${rootRotData?.risk_level === 'High' ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'} rounded-lg shadow-md p-4 text-center border-2`}>
              <div className="flex justify-center mb-2">
                <svg className={`w-8 h-8 ${rootRotData?.risk_level === 'High' ? 'text-red-500' : 'text-green-500'}`} fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                </svg>
              </div>
              <p className="text-gray-700 font-semibold">Root Rot Prediction</p>
              <p className={`${rootRotData?.risk_level === 'High' ? 'text-red-500' : 'text-green-500'} text-2xl font-bold`}>
